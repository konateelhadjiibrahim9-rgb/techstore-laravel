                            ...
                            @foreach($recentQuotes as $quote)
                                <div class="flex items-center justify-between p-4 bg-[#f8fafc] rounded-lg">
                                    ...
                                    <span class="px-3 py-1 rounded-full text-xs font-medium
                                        {{ ($quote->status === 'pending') ? 'bg-yellow-100 text-yellow-800' : (($quote->status === 'approved') ? 'bg-green-100 text-green-800' : (($quote->status === 'rejected') ? 'bg-red-100 text-red-800' : 'bg-blue-100 text-blue-800')) }}">
